...
\item Make sure you are {\bf cd}'ed into the target's directory by typing \textbf{chdir} and identifying the last directory in the path as the target directory. Once confirmed, type the command {\bf edit input-file.txt}. Now you will begin editing the file.
\item First, enter the absolute file paths for the dark calibration files in the {\bf DIRDARK} keyword. The reason there is a separate keyword for the dark calibration images is because it is common that darks are not taken on the same night as flats and biases. Therefore, these may or may not be stored in the same location. If they are stored in the same directory as the other calibration files (in the target directory), just leave this entry blank.
...
\item Move this image into the directory with your instrument signature removed images by typing {\bf mv /home/Downloads/new-image.fits /home/depot/STEPUP/raw/\emph{name-of-target}/\emph{date-of-observation}/ISR\_Images/}. \textcolor{red}{Type it where? I would clarify that}. Now that this image is the correct directory, enter {\bf Y} to continue.
\end{enumerate}
...
If your data has been recorded in the Allegheny Observatory Server, but has not yet been transferred to the Astro Computing Server in Allen 302, follow the directions below to do so. The directory in these computers where all ASTRON 1263 data is stored is {\bf /home/depot/A1263}. \newline

\begin{enumerate}
\item Log onto Ptolemy in Allen 302. Open up a terminal window by clicking the terminal icon at the top of the screen. If a directory for your target does not exist, use the {\bf mkdir} command to first make a directory for your target in {\bf /home/depot/A1263} by typing {\bf mkdir /home/depot/A1263/\emph{date-of-observation}}. You may possibly need to specialize the sub-directory further than the date, perhaps by adding one of your group-member's initials if this sub-directory name has already been taken.
...
\item A template of the input file is provided in SIA. Copy the input file template to the target directory by typing {\bf cp /home/depot/STEPUP/} {\bf input-file.txt /home/depot/STEPUP/ \newline raw/\emph{target-name}/\emph{date-of-observation}}. 
...
\label{sect:newim302}
\begin{enumerate}
\item Once you have created the \emph{new-image.fits} file as described in Section \ref{sect:newim}, it will be saved in your downloads folder. If you have already ran ISR, open up a new terminal window and type the command {\bf mv Downloads/new-image.fits /home/depot/A1263/\emph{data-subdirectory}/ISR\_Images}. If you have not yet run ISR, enter {\bf mv Downloads/new-image.fits /home/depot/A1263/\emph{data-subdirectory}} and be sure to move the file to \emph{ISR\_Images} before running \emph{ASTROM}.
\end{enumerate}
...
\item Next, in the same terminal window type {\bf cd /home/depot/STEPUP/STEPUP\_image}{\bf\_analysis/}. Now that you have entered into the directory containing the code, type the command {\bf python3 main.py}. This will begin running the code.
...
\item Now, SIA should be running whichever function you entered. Note that if you are running \emph{ASTROM}, you will first be prompted on whether you have saved a \emph{new-image.fits} file to the appropriate directory, as specified in Sections \ref{sect:newim} and \ref{sect:newim302}. If so, enter {\bf Y}. If not, generate one as described in Sections \ref{sect:newim} and \ref{sect:newim302} and save it to the appropriate directory and then enter {\bf Y}.
